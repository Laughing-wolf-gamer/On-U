        ...
        >
            ...
            {featuresList && featuresList.length > 0 && featuresList.map((slide, index) => (
                <img
                    ...
                    alt={`Slide ${index + 1}`}
                    draggable = {false}
                    className={`cursor-grab absolute top-0 left-0 w-full h-full object-cover rounded-[30px] transition-opacity duration-700 ease-in-out ${
            ...
            <div className="absolute bottom-4 flex space-x-2">
                {featuresList.map((_, dotIndex) => (
                    <div
                        ...
                        className={`w-3 h-3 rounded-full cursor-pointer ${currentSlide === dotIndex ? 'bg-blue-500' : 'bg-gray-300'}`}
                    />
                ))}
            </div>
        </div>
