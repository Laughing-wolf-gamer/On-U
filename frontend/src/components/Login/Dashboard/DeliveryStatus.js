...
const DeliveryStatus = ({ status, hiddenText }) => {
  ...
  if (currentStepIndex < 0) {
    ...
    currentStepIndex = steps.length; // The new "Unknown" step will be at the end
  }
        ...
        <div className="flex items-center justify-between relative overflow-hidden">
          ...
          {currentStepIndex === steps.length && (
            <div className="flex flex-col items-center z-10 overflow-hidden">
              ...
              >
                <div
                  className={`rounded-full flex items-center justify-center bg-yellow-500 w-10 h-10 transform transition-transform duration-500 ease-in-out`}
                >
                  ❓
                </div>
                {!hiddenText && (
                  <p className="mt-2 text-xs font-semibold text-gray-700">Unknown Status</p>
                )}
              </div>
            </div>
          )}
