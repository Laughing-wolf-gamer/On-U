...
const SideBarBag = () => {
	...
	return (
		<div className="flex flex-row justify-start items-start min-h-screen gap-4 p-4">
			{/* Left Section (Product Listing) */}
			<div className="w-[30%] px-3 border-r border-gray-800 flex flex-col items-center">
				{randomProducts && randomProducts.length > 0 && (
				<div className="flex flex-col items-center w-full">
					<h1 className="text-center mt-4 font-medium text-xl text-gray-800">DISCOVER MORE</h1>
					<ul className="grid grid-cols-1 gap-3 max-h-[650px] overflow-y-auto w-full">
			...
			<div className="w-[70%] flex justify-self-start flex-col h-full justify-start items-center">
				<div className='w-full max-h-[580px] overflow-y-auto'>
					<OfflineBagContent
		...
		{[1, 2, 3, 4, 5, 5].map((item, i) => {
			...
			<div key={i} className="flex flex-col items-start border-b py-4 space-y-2 sm:space-x-4 sm:space-y-0">
				...
				<div className="flex flex-row justify-start items-start space-x-2">
					...
					{/* Quantity Selector */}
					<div className="mt-2 flex items-center space-x-3 shadow-sm rounded-full border-gray-700 border">
						<div className="flex items-center space-x-1 justify-between">
						<button
							onClick={() => updateQty({ target: { value: Math.max(active?.quantity - 1, 1) } }, active.ProductData._id)}
							className="h-8 w-8 px-2 rounded-full text-xs sm:text-sm disabled:text-gray-300"
